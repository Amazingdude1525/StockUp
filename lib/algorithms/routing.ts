...
export function aStar(
  ...
  while (open.size) {
    const current = [...open].sort(
      (a, b) =>
        (g.get(a) ?? Infinity) +
        euclidean(byId.get(a)!, goal) -
        ((g.get(b) ?? Infinity) + euclidean(byId.get(b)!, goal)),
    )[0];
    if (current === goalId) {
      const path = [current];
      let cursor = current;
    ...
    for (const edge of edges.filter(
      (e) =>
        !e.blocked &&
        (e.from === current || (e.bidirectional !== false && e.to === current)),
    )) {
...
export const routeDistance = (route: Point[]) =>
  route.slice(1).reduce((d, p, i) => d + euclidean(route[i], p), 0);
export function nearestNeighbor(start: Point, stops: Point[]) {
  const left = [...stops],
    route = [start];
  while (left.length) {
...
export function warehouseAStarRoute(stops: Point[]) {
  ...
  stops.forEach((stop, i) => {
    const corridorY = stop.y + 65,
      access = { id: `A-${i}`, x: stop.x + 37, y: corridorY },
      goal = { id: `G-${i}`, x: stop.x + 37, y: stop.y + 21 };
